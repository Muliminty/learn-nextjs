...
- **使用 `[param]`**：当你知道确切的路由段数量时（如 `/blog/[slug]`）
- **使用 `[...slug]`**：当你需要捕获多个不确定的路由段，但至少需要 1 个时（如文档系统）
- **使用 `[[...slug]]`**：当你需要同时支持根路径和子路径时（如电商分类页面）

### 常见错误

1. ❌ **忘记 `page.tsx`**：没有 `page.tsx` 的文件夹不会创建路由
2. ❌ **混淆 `[...slug]` 和 `[[...slug]]`**：注意双括号表示可选
3. ❌ **参数类型错误**：`[...slug]` 返回数组，不是字符串
4. ❌ **在路由组中使用动态路由**：路由组 `(folder)` 不影响 URL，但可以在其中使用动态路由
